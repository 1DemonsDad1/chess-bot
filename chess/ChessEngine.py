...
class GameState:
    ...
    def makeMove(self, move):
        ...
        if move.is_castle_move:  # king-side castle move
    ...
    def getValidMoves(self):
        ...
        if self.in_check:
            if len(self.checks) == 1:  # only 1 check, block the check or move the king
                moves = self.advanced_check(king_row, king_col)
    ...
    def advanced_check(self, king_row, king_col):
        result = self.getAllPossibleMoves()
        ...
        for i in range(len(result) - 1, -1, -1):  # iterate through the list backwards when removing elements
            if (
                result[i].piece_moved[1] != "K"
                and (result[i].end_row, result[i].end_col) not in valid_squares
            ):
    ...
    def squareUnderAttack(self, row, col):
        ...
        return any(
            move.end_row == row and move.end_col == col for move in opponents_moves
        )
            ...
            for i in range(1, 8):
                ...
                if end_piece[0] == ally_color and end_piece[1] != "K":
                    ...
                    if (0 <= j <= 3 and enemy_type == "R") or (4 <= j <= 7 and enemy_type == "B") or (
                                ...
                                enemy_type == "Q") or (i == 1 and enemy_type == "K"):  # no piece blocking, so check
                        if possible_pin == ():
                            in_check = True
    ...
    def getPawnMoves(self, row, col, moves):
        ...
        if self.board[row + move_amount][col] == "--" and (
            not piece_pinned or pin_direction == (move_amount, 0)
        ):
            moves.append(Move((row, col), (row + move_amount, col), self.board))
            if row == start_row and self.board[row + 2 * move_amount][col] == "--":  # 2 square pawn advance
                moves.append(Move((row, col), (row + 2 * move_amount, col), self.board))
        if col >= 1 and (not piece_pinned or pin_direction == (move_amount, -1)):
        ...
        if col <= 6 and (not piece_pinned or pin_direction == (move_amount, +1)):
            if self.board[row + move_amount][col + 1][0] == enemy_color:
                moves.append(Move((row, col), (row + move_amount, col + 1), self.board))
            if (row + move_amount, col + 1) == self.enpassant_possible:
                attacking_piece = blocking_piece = False
...
class Move:
    ...
    def getChessNotation(self):
        if self.is_pawn_promotion:
            return f"{self.getRankFile(self.end_row, self.end_col)}Q"
        if self.is_castle_move:
            return "0-0-0" if self.end_col == 1 else "0-0"
        if self.is_enpassant_move:
            return f"{self.getRankFile(self.start_row, self.start_col)[0]}x{self.getRankFile(self.end_row, self.end_col)} e.p."
        if self.piece_captured != "--":
            return (
                f"{self.getRankFile(self.start_row, self.start_col)[0]}x{self.getRankFile(self.end_row, self.end_col)}"
                if self.piece_moved[1] == "p"
                else f"{self.piece_moved[1]}x{self.getRankFile(self.end_row, self.end_col)}"
            )
        if self.piece_moved[1] == "p":
    ...
    def __str__(self):
        ...
        if self.piece_moved[1] == "p":
            if self.is_capture:
                ...
                return f"{end_square}Q" if self.is_pawn_promotion else end_square
