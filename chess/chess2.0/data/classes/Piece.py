...
class Piece:
	...
	def move(self, board, square, force=False):	
		# sourcery skip: assign-if-exp, extract-duplicate-method, extract-method, merge-comparisons, split-or-ifs, swap-nested-ifs
		...
		if square in self.get_valid_moves(board) or force:
			...
			# Pawn promotion and ep myb
			if self.notation == ' ':
				if self.ep_pawn:
					if self.color=="white":
						square_ep=board.get_square_from_pos((self.x,self.y+1))
					else:
						square_ep=board.get_square_from_pos((self.x,self.y-1))
					square_ep.occupying_piece=None
				#promotion
